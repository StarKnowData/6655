...
class openconnect extends top {
	...
	/* 连接qq */
	public function qq() {
		if ($this->yb ['openlogin_qq_open'] == 0) {
		...
		if ($this->spArgs ( 'login' )) {
			...
			$user = spClass ( 'db_memberex' )->spLinker ()->find ( array (
			...
			if ($user ['expires'] != 0 && time () > $user ['expires']) {
				...
				spClass ( 'db_memberex' )->delete ( array (
						'openid' => $this->user ['openid'] 
				) );
	...
	public function weibo() {
		if ($this->yb ['openlogin_weib_open'] == 0) {
			exit ( '系统管理员没有开启微博登陆功能' );
		}
		$this->app = 'weibo';
		...
		if ($this->spArgs ( 'callback' )) {
			if (spClass ( 'sinaConnect' )->LoginCallback ()) {
				header ( "Location:index.php?c=openconnect&a=weib&login=yes" );
			}
			exit ();
		}
	...
	private function activeLogin($params) {
		...
		$result = spClass ( 'db_member' )->find ( array (
				'uid' => $params ['uid'] 
		) );
	...
	private function _getActionToken($uid) {
		$rs = spClass ( 'db_memberex' )->spLinker ()->findAll ( array (
				'uid' => $uid 
		) );
		foreach ( $rs as $d ) {
