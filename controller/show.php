...
class show extends top {
	public function index() {
		...
		$this->jj = spClass ( 'db_blog' )->findAll ( array (
				'type' => 1,
				'`show`' => '1',
				'top' => '1' 
		), 'bid desc', '*', 5 );
		 ...
		 * 广告位
		 */
		$this->banner = spClass ( 'db_banner' )->findAll ( array (
				'fid' => 1 
		), '`order`' );
		$this->xiaobao = spClass ( 'db_banner' )->find ( array (
				'fid' => 2 
		), '`order`' );
		$this->jigou = spClass ( 'db_banner' )->findAll ( array (
				'fid' => 3 
		), '`order`' );
		$this->daren1 = spClass ( 'db_banner' )->find ( array (
				'fid' => 4 
		), '`order`' );
		$this->daren2 = spClass ( 'db_banner' )->find ( array (
				'fid' => 5 
		), '`order`' );
	...
	public function jujiao() {
		...
		$this->sort_id = $sort_id;
		$s_channel = spClass ( 'db_sort' )->find ( array (
				'fid' => $sort_id 
		), '`order`' );
		$this->fid = $s_channel ['id'];
		$n_id = isset ( $_GET ['nid'] ) ? intval ( $_GET ['nid'] ) : $s_channel ['id'];
		$this->n_id = $n_id;
		$blogs = spClass ( 'db_blog' )->spPager ( $this->spArgs ( 'page', 1 ), 10 )->findAll ( array (
				'type' => 1,
				'`show`' => '1',
				'sort_id' => $n_id 
		), 'bid desc' );
		$this->blogs = $blogs;
		$this->pager = spClass ( 'db_blog' )->spPager ()->pagerHtml2 ( 'show', 'jujiao', array (
				'sid' => $sort_id,
				'nid' => $n_id 
		) );
		$page = spClass ( 'db_blog' )->spPager ()->getPager ( '' );
		$images = array ();
		$blogs_1 = spClass ( 'db_blog' )->spPager ( $this->spArgs ( 'page', 1 ), 10 )->findAll ( array (
				'type' => 1,
				'`show`' => '1',
				'sort_id' => $n_id,
				'top' => '1' 
	...
	public function jigou91() {
		...
		$this->blogs = $blogs;
		$this->pager = spClass ( 'db_blog' )->spPager ()->pagerHtml2 ( 'show', 'jigou91', array (
				'sid' => $sort_id,
				'nid' => $n_id 
		) );
		$page = spClass ( 'db_blog' )->spPager ()->getPager ( '' );
	...
	public function saishihd() {
		...
		$this->nav = $nav;
		$this->obj = $obj;
		$this->dance = spClass ( 'db_sort' )->findAll ( array (
				'fid' => 22 
		), '`order`', '*', 3 );
		$this->breakin = spClass ( 'db_sort' )->findAll ( array (
				'fid' => 129 
		), '`order`', '*', 3 );
		$this->qiwu = spClass ( 'db_sort' )->findAll ( array (
				'fid' => 130 
		), '`order`', '*', 3 );
	...
	public function saishimore() {
		...
		$this->blogs = spClass ( 'db_blog' )->spPager ( $this->spArgs ( 'page', 1 ), 10 )->findAll ( array (
				'type' => 4,
				'`show`' => 1,
				'sort_id' => $sid 
		) );
	...
	public function v() {
		...
		$this->d = spClass ( 'db_blog' )->spLinker ()->find ( array (
				'uid' => $this->user_data ['uid'],
				'bid' => $this->spArgs ( 'bid' ) 
		) );
		
		if (is_array ( $this->d )) {
			spClass ( 'db_blog' )->incrField ( array (
					'bid' => $this->spArgs ( 'bid' ) 
			), 'hitcount' );
