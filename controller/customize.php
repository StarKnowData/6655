...
class customize extends top {
	...
	public function index() {
		$rs = spClass ( 'db_theme' )->find ( array (
				'uid' => $this->uid 
		) ); // 加载我的主题设置
	...
	public function savetheme() {
		...
		$conditions = array (
				...
				'config' => serialize ( $_SESSION ['customize'] ['config'] ),
				'setup' => serialize ( $_SESSION ['customize'] ['setup'] ) 
		);
		spClass ( 'db_theme' )->replace ( $where, $conditions );
		spClass ( 'db_skins' )->incrField ( array (
				'skindir' => $_SESSION ['customize'] ['theme'] 
		), 'usenumber' );
		$_SESSION ['customize'] = '';
		...
		header ( "Location:" . spUrl ( 'userblog', 'index', array (
				'domain' => 'home',
				'uid' => $this->uid 
		) ) );
	}
	...
	function preview() {
		if ($this->spArgs ( 'skinid' ) == 'default') {
			...
			$theme = spClass ( 'db_skins' )->find ( array (
					'id' => $this->spArgs ( 'skinid' ) 
			) );
		...
		if (is_array ( $_FILES )) {
			foreach ( $_FILES as $k => $d ) {
				...
				if ($d ['error'] != 4) {
					...
					if ($this->yb ['theme_upload'] != 1) {
						continue;
					}
					if ($d ['size'] > $this->yb ['theme_uploadsize']) {
						exit ( '上传大小不能超过：' . ($this->yb ['theme_uploadsize'] / 1024 / 1024) . 'MB' );
					}
					$ext = spExt ( 'aUpload' );
					...
					if (! in_array ( $ext ['extension'], $exrarr )) {
						exit ( '只能上传：' . $this->yb ['theme_uploadtype'] );
					}
