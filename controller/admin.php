...
class admin extends top {
	...
	function system() {
		if ($this->spArgs ( 'submit' )) {
			...
			$this->jump ( spUrl ( 'admin', 'system', array (
					'ac' => 'ok' 
			) ) );
		}
	...
	function models() {
		...
		if ($this->spArgs ( 'setup' )) {
			if ($this->spArgs ( 'submit' )) {
				...
				spClass ( 'db_models' )->update ( array (
						'id' => $this->spArgs ( 'setup' ) 
				), $row );
				spClass ( 'db_cache' )->delete ();
				$this->jump ( spUrl ( 'admin', 'models', array (
						'ac' => 'ok' 
				) ) );
			}
			$this->minfo = spClass ( 'db_models' )->find ( array (
					'id' => $this->spArgs ( 'setup' ) 
			) );
	...
	function blog() {
		...
		if ($this->spArgs ( 'submit' )) {
			$this->pager = spClass ( 'db_blog' )->spPager ()->pagerHtml ( 'admin', 'blog', array (
					'title' => $title,
					'niname' => $niname,
	...
	function user() {
		if ($this->spArgs ( 'mod' ) == 'info') {
			...
			if ($this->spArgs ( 'delface' )) {
				...
				@unlink ( $path2 );
				@unlink ( $path2 );
				$this->jump ( spUrl ( 'admin', 'user', array (
						'mod' => 'info',
						'uid' => $uid,
						'ac' => 'ok' 
				) ) );
			...
			$this->info = spClass ( 'db_member' )->find ( array (
					'uid' => $this->spArgs ( 'uid' ) 
			) );
	...
	function notice() {
		if ($this->spArgs ( 'addnotice' )) {
			if ($this->spArgs ( 'title' ) == '' || $this->spArgs ( 'info' ) == '') {
				$this->jump ( spUrl ( 'admin', 'notice', array (
						'ac' => 'err' 
				) ) );
			}
		...
		if ($this->spArgs ( 'clear' )) {
			spClass ( 'db_notice' )->delete ( array (
					'sys' => $this->spArgs ( 'clear' ) 
			) );
			$this->success ( '删除成功', spUrl ( 'admin', 'notice' ) );
		}
	...
	function systag() {
		if ($this->spArgs ( 'syscate' )) {
			...
			$this->jump ( spUrl ( 'admin', 'systag', array (
					'ac' => 'ok' 
			) ) );
		}
		if ($this->spArgs ( 'sysadd' )) {
			...
			spClass ( 'db_category' )->create ( array (
					'sort' => $this->spArgs ( 'sort' ),
					'catename' => $this->spArgs ( 'cname' ) 
			) );
			$this->jump ( spUrl ( 'admin', 'systag', array (
					'ac' => 'ok' 
			) ) );
	...
	function usertag() {
		if ($this->spArgs ( 'rebuilt' )) {
			...
			$this->jump ( spUrl ( 'admin', 'usertag', array (
					'ac' => 'ok' 
			) ) );
		}
	...
	function recommend() {
			...
			if (is_array ( $this->spArgs ( 'delall' ) ) || is_array ( $this->spArgs ( 'opened' ) )) {
				...
				if (is_array ( $this->spArgs ( 'delall' ) )) {
					foreach ( $this->spArgs ( 'delall' ) as $d ) {
						spClass ( 'db_recommend' )->delete ( array (
								'id' => $d 
						) );
					}
				}
				...
				if (is_array ( $this->spArgs ( 'opened' ) )) {
					foreach ( $this->spArgs ( 'opened' ) as $d ) {
						spClass ( 'db_recommend' )->update ( array (
								'id' => $d 
						), array (
								'open' => 1 
						) );
		...
		if ($this->spArgs ( 'filter' )) {
			$this->pager = spClass ( 'db_recommend' )->spPager ()->pagerHtml ( 'admin', 'recommend', array (
					'filter' => $this->spArgs ( 'filter' ),
	...
	function theme() {
		if ($this->spArgs ( 'm' ) == 'info') {
			if ($this->spArgs ( 'submit' )) {
				spClass ( 'db_skins' )->update ( array (
						'id' => $this->spArgs ( 'id' ) 
				), $this->spArgs () );
				$this->success ( '保存成功', spUrl ( 'admin', 'theme' ) );
			}
			$this->skin = spClass ( 'db_skins' )->find ( array (
					'id' => $this->spArgs ( 'id' ) 
			) );
		...
		if ($this->spArgs ( 'm' ) == 'install') {
			...
			$result = spClass ( 'db_skins' )->find ( array (
		...
		if ($this->spArgs ( 'm' ) == 'uninstall') {
			$rs = spClass ( 'db_skins' )->find ( array (
					'id' => $this->spArgs ( 'id' ) 
			) );
			spClass ( 'db_skins' )->delete ( array (
					'id' => $this->spArgs ( 'id' ) 
			) );
			spClass ( 'db_theme' )->delete ( array (
					'theme' => $rs ['skindir'] 
			) ); // 将用户设置的删掉
			if ($rs ['skindir']) {
	...
	public function cpage() {
		...
		if ($id = $this->spArgs ( 'editCategory' )) {
			if ($this->spArgs ( 'post' )) {
				...
				if (true == $db_cpage_cate->editCate ( $this->spArgs (), array (
						'id' => ( int ) $id 
				) ))
			...
			$this->ass = $db_cpage_cate->find ( array (
					'id' => $id 
			) );
		...
		if ($id = $this->spArgs ( 'addBody' )) {
			...
			$this->body = $db_cpage_body->find ( array (
					'cid' => $cid 
			) );
	...
	public function adUnit() {
		...
		if ($this->spArgs ( 'flag' )) {
			$this->ClearSpAccess ();
			if (true == $db_ad_unit->changeShow ( array (
					'is_show' => ( int ) $this->spArgs ( 'en_show' ) 
			), array (
	...
	public function adContent() {
		...
		if ($this->spArgs ( 'flag' )) {
			$this->ClearSpAccess ();
			if (true == $db_ad_list->changeShow ( array (
					'is_show' => ( int ) $this->spArgs ( 'en_show' ) 
			), array (
					'adid' => ( int ) $this->spArgs ( 'id' ) 
			) )) {
				$this->success ( '修改成功' );
			} else {
				$this->error ( '修改失败' );
	...
	private function upLoadPic() {
		...
		if ($file ['error'] == 2)
			return array (
					'error' => '传文件的大小超过了 HTML 表单中 MAX_FILE_SIZE 选项指定的值' 
			);
		if ($file ['error'] == 3)
			return array (
					'error' => '文件只有部分被上传' 
		...
		if (move_uploaded_file ( $file ['tmp_name'], $addir . $name )) {
			return array (
					'error' => '',
					'img' => $Relative_path . $name 
			);
		} else {
			return array (
					'error' => '移动文件失败，请检查' . $addir . '目录权限' 
			);
		}
	}
	...
	function banner() {
		...
		if ($this->spArgs ( 'mod' ) == 'del') {
			$id = $this->spArgs ( 'id' );
			spClass ( 'db_banner' )->delete ( array (
					'id' => $id 
			) );
