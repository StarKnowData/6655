...
class userblog extends top {
	...
	public function index() {
		...
		$this->follow = spClass ( 'db_follow' )->spLinker ()->findAll ( array (
				'uid' => $this->user_data ['uid'] 
		), 'time desc', '', '20' ); // 显示我关注的20个
	...
	private function getMyFollow() {
		$this->follow = spClass ( 'db_follow' )->spLinker ()->findAll ( array (
				'uid' => $this->user_data ['uid'] 
		), 'time desc', '', '24' ); // 显示我关注的24个
	}
	
	/* 谁关注我 */
	private function getMyLook() {
		$this->myLook = spClass ( 'db_follow' )->findCount ( array (
	...
	private function isFollow() {
		...
		$follow = spCLass ( 'db_follow' )->find ( array (
				'uid' => $_SESSION ['uid'],
				'touid' => $this->user_data ['uid'] 
		) );
		if (is_array ( $follow )) {
	...
	private function getBlogFava() {
		...
		$count = spClass ( 'db_likes' )->findCount ( array (
				'bid' => $bid 
		) );
		$data ['rs'] = $rs;
		$data ['count'] = $count;
	...
	private function getUserSkin($bid = 0) {
		...
		$skin = spClass ( 'db_theme' )->find ( array (
				'uid' => $rs ['uid'] 
		) );
		if ($skin ['setup'])
		...
		if ($_SESSION ['customize'] ['theme'] != '') // 如果自定义有数据则启用自定义的界面
{
		...
		if ($_SESSION ['customize'] ['css'] != '')
			$css = $_SESSION ['customize'] ['css']; // 如果session的css不为空则调用预览模式的css
