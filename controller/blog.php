...
class blog extends top {
	...
	function getyytimg() {
		...
		if (strpos ( $this->spArgs ( 'src' ), 'yyt' ) || strpos ( $this->spArgs ( 'src' ), 'yinyuetai' )) {
			echo spClass ( 'urlParse' )->getRefereData ( $url, 'http://www.yinyuetai.com/' );
			exit ();
		}
	...
	function readnotice() {
		...
		echo spClass ( 'db_notice' )->update ( array (
				'uid' => $this->uid,
				'id' => $this->spArgs ( 'id' ) 
		), array (
				'isread' => 1 
	...
	function delnotice() {
		...
		$rs = spClass ( 'db_notice' )->find ( array (
				'id' => $id 
		) );
		if ($rs ['uid'] == $this->uid || $rs ['foruid'] == $this->uid)
			echo spClass ( 'db_notice' )->delete ( array (
					'id' => $id 
			) );
	}
	function tuijian() {
		...
		if ($this->spArgs ( 'submit' )) {
			...
			if ($this->err == '') {
				exit ( '<script>parent.tuijianok();</script>' );
			}
		}
