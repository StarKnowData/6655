...
class login extends top {
	...
	/** 用户登录 */
	function vary() {
		if ($this->spArgs ( 'email' ) == '' || $this->spArgs ( 'password' ) == '')
			return $this->api_error ( '用户名密码不能为空' );
		...
		if (! is_array ( $rs ))
			$this->api_error ( '用户名不存在' );
		if ($rs ['open'] == 0)
			$this->api_error ( '该帐号被限制登录' );
	...
	function reg() {
		if ($this->spArgs ( 'email' ) == '' || $this->spArgs ( 'password' ) == '' || $this->spArgs ( 'username' ) == '')
			return $this->api_error ( '用户名、密码、昵称不能为空' );
		if (strlen ( $this->spArgs ( 'email' ) ) < 5 || strlen ( $this->spArgs ( 'email' ) ) > 30)
			return $this->api_error ( '邮箱必须大于5小于30个字符' );
		if (! validateEmail ( $this->spArgs ( 'email' ) ))
			return $this->api_error ( '邮箱格式不符合规范' );
		if (strlen ( $this->spArgs ( 'username' ) ) < 2 || strlen ( $this->spArgs ( 'username' ) ) > 12)
			return $this->api_error ( '昵称最短为2个字符最长为12个字符' );
		if (strlen ( $this->spArgs ( 'password' ) ) < 6)
			return $this->api_error ( '密码最少6位' );
		...
		if (is_array ( $result )) {
			return $this->api_error ( '注册邮箱已经存在,请更换邮箱' );
		}
		$result = spClass ( 'db_member' )->findBy ( 'username', $this->spArgs ( 'username' ) );
		if (is_array ( $result )) {
			return $this->api_error ( '昵称已经存在,请更换昵称' );
		}
	...
	function findpwd() {
		if ($this->spArgs ( 'token' ) != '') {
			...
			if (is_array ( $rs )) {
				if ($this->spArgs ( 'password' ) == '' || strlen ( $this->spArgs ( 'password' ) ) < 6)
					$this->api_error ( '密码最小6位' );
				if ($this->spArgs ( 'password' ) != $this->spArgs ( 'password2' ))
					$this->api_error ( '两次密码不一样请检查' );
		...
		if ($this->spArgs ( 'email' ) == '' || strlen ( $this->spArgs ( 'email' ) ) < 5)
			$this->api_error ( '请输入正确的邮箱' );
		if (! spClass ( 'spVerifyCode' )->verify ( $this->spArgs ( 'verifycode' ) ))
			$this->api_error ( '验证码不正确' );
		$result = spClass ( 'db_member' )->findBy ( 'email', $this->spArgs ( 'email' ) );
		...
		if (is_array ( $result )) {
			...
			if ($row = spClass ( 'db_findpwd' )->checkToken ( $result ['uid'] )) { // 如果已经有记录且不过期
				if ($row ['mailsend'] > time ())
					$this->api_error ( '您已经提交过请求，下次请求请在' . date ( 'Y-m-d H:i:s', $row ['mailsend'] ) . '后执行' );
				spClass ( 'db_findpwd' )->updateMailsendTime ( $result ['uid'] );
				spClass ( 'db_notice' )->sendFindPwd ( $row ['uid'], $url . spURl ( 'main', 'resetpwd', array (
						'token' => $row ['token'] 
				) ) );
			} else {
				$row = spClass ( 'db_findpwd' )->createToken ( $result ['uid'] );
				spClass ( 'db_findpwd' )->updateMailsendTime ( $result ['uid'] ); // 更新邮件发送时间
				spClass ( 'db_notice' )->sendFindPwd ( $row, $url . spURl ( 'main', 'resetpwd', array (
						'token' => $row ['token'],
						'do' => 'submit' 
				) ) );
			}
