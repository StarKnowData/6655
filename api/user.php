...
class user extends top {
	...
	function save_baseinfo() {
		if ($this->yb ['keep_niname'] != '') {
			...
			if (in_array ( $this->spArgs ( 'niname' ), $arr )) {
				$this->api_error ( '该昵称被保留或限制' );
			}
		}
		
		if ($this->yb ['keep_domain'] != '') {
			$arr = explode ( ',', $this->yb ['keep_domain'] );
			if (in_array ( $this->spArgs ( 'domain' ), $arr )) {
				$this->api_error ( '该个性域名被保留或限制' );
			}
		}
		
		if (utf8_strlen ( $this->spArgs ( 'niname' ) ) < 2 || utf8_strlen ( $this->spArgs ( 'niname' ) ) > 10) {
			$this->api_error ( '昵称最短2位最长10位' );
		}
		$niname = spClass ( 'db_member' )->find ( array (
				'username' => $this->spArgs ( 'niname' ) 
		), '', 'uid,username' );
		if (is_array ( $niname ) && $niname ['uid'] != $this->uid) {
			$this->api_error ( '该昵称已被使用' );
		} // 判断昵称是否被使用
		if (utf8_strlen ( $this->spArgs ( 'domain' ) ) < 4 || utf8_strlen ( $this->spArgs ( 'domain' ) ) > 15) {
			$this->api_error ( '个性域名最短4位最长15位' );
		}
		if (! preg_match ( '/^[a-zA-Z]{1}([a-zA-Z0-9]|[._]){1,15}$/', $this->spArgs ( 'domain' ) )) {
			$this->api_error ( '个性域名不符合要求' );
		}
		$domain = spClass ( 'db_member' )->find ( array (
				'domain' => $this->spArgs ( 'domain' ) 
		), '', 'uid,domain' );
		if (is_array ( $domain ) && $domain ['uid'] != $this->uid) {
			$this->api_error ( '个性域名已被使用' );
		} // 判断个性域名是否被使用
		if ($this->spArgs ( 'tag' ) != '') {
		...
		if ($this->yb ['mail_open'] == 1) {
			...
			if ($this->spArgs ( 'm_fows' ) == 1) {
				$_mfow = 1;
			} else {
				$_mfow = 0;
			}
			if ($this->spArgs ( 'm_pms' ) == 1) {
				$_mpm = 1;
			} else {
				$_mpm = 0;
			}
		...
		if (spClass ( 'db_member' )->update ( array (
				'uid' => $this->uid 
		), $row )) {
	...
	function save_password() {
		...
		if ($user ['password'] != $localpwd) {
			...
			spClass ( 'db_member' )->update ( array (
					'uid' => $this->uid 
			), $row );
	...
	function mylikes() {
		...
		foreach ( $data ['blog'] as &$d ) {
			$d ['h_url'] = goUserHome ( array (
					'uid' => $d ['uid'],
					'domain' => $d ['domain'] 
			) );
			$d ['h_img'] = avatar ( array (
					'uid' => $d ['uid'],
					'size' => 'middle' 
			) );
			$d ['b_url'] = goUserBlog ( array (
					'bid' => $d ['bid'],
					'domain' => $d ['domain'],
					'uid' => $d ['uid'] 
			) );
			...
			$d ['repto'] = $rs ['repto'];
			if (! empty ( $d ['repto'] )) {
				$d ['repto'] ['h_url'] = goUserHome ( array (
						'uid' => $d ['repto'] ['uid'],
						'domain' => $d ['repto'] ['domain'] 
				) );
				$d ['repto'] ['h_img'] = avatar ( array (
						'uid' => $d ['repto'] ['uid'],
						'size' => 'small' 
				) );
			} else {
	...
	public function myfollow() {
		...
		foreach ( $data ['data'] as &$d ) {
			...
			unset ( $d ['meto'], $d ['tome'] );
			$tudo ['h_url'] = goUserHome ( array (
					'uid' => $tudo ['uid'],
					'domain' => $tudo ['domain'] 
			) );
			$tudo ['h_img'] = avatar ( array (
					'uid' => $tudo ['uid'],
					'size' => 'middle' 
			) );
	...
	function myreply() {
		...
		foreach ( $data ['data'] as &$d ) {
			...
			$d ['me'] ['h_url'] = goUserHome ( array (
					'uid' => $d ['me'] ['uid'],
					'domain' => $d ['me'] ['domain'] 
			) );
			$d ['me'] ['h_img'] = avatar ( array (
					'uid' => $d ['me'] ['uid'],
					'size' => 'middle' 
			) );
			$d ['to'] ['h_url'] = goUserHome ( array (
					'uid' => $d ['to'] ['uid'],
					'domain' => $d ['to'] ['domain'] 
			) );
			$d ['to'] ['h_img'] = avatar ( array (
					'uid' => $d ['to'] ['uid'],
					'size' => 'middle' 
			) );
			$d ['blog'] ['b_url'] = goUserBlog ( array (
					'bid' => $d ['blog'] ['bid'] 
			) );
			$d ['time'] = ybtime ( array (
					'time' => $d ['time'] 
	...
	public function mynotice() {
		...
		if (! empty ( $rs )) {
			foreach ( $rs as $d ) {
				$d ['user'] ['h_url'] = goUserHome ( array (
						'uid' => $d ['user'] ['uid'],
						'domain' => $d ['user'] ['domain'] 
				) );
				$d ['user'] ['h_img'] = avatar ( array (
						'uid' => $d ['user'] ['uid'],
						'size' => 'small' 
				) );
				$d ['time'] = ybtime ( array (
				...
				if ($href [0] == 'blog') {
					$d ['location'] = goUserBlog ( array (
							'bid' => $href [1] 
					) );
				}
				if ($href [0] == 'user') {
					$d ['location'] = goUserHome ( array (
							'uid' => $href [1] 
					) );
				}
	...
	public function chkNoticPm() {
		$data = array ();
		if (islogin ()) {
			$data ['notic'] = spClass ( 'db_notice' )->findCount ( array (
					'foruid' => $this->uid,
					'isread' => 0 
			) );
			$data ['pm'] = spCLass ( 'db_pm' )->findCount ( array (
					'touid' => $this->uid,
					'isnew' => 1 
			) );
		} else {
	...
	function fllow() {
		...
		if (! spClass ( 'db_member' )->find ( array (
				'uid' => $this->spArgs ( 'uid' ) 
		) )) {
	...
	function wizard_save_follow() {
		if ($this->spArgs ( 'uid' ) != '') {
			...
			foreach ( $uid as $d ) {
				$uids = intval ( $d );
				if ($uids != '')
					spClass ( 'db_follow' )->changeFollow ( 'link', $uids, $this->uid );
			}
			$this->api_success ( true );
		}
		$this->api_error ( '系统繁忙' );
