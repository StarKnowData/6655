...
class blog extends top {
		...
		foreach ( $tags as &$d ) {
			...
			if (is_array ( $d ['ulist'] )) {
				foreach ( $d ['ulist'] as &$list ) {
					$list ['h_url'] = goUserHome ( array (
							'uid' => $list ['uid'],
							'domain' => $list ['domain'] 
					) );
					$list ['h_img'] = avatar ( array (
							'uid' => $list ['uid'],
							'size' => 'small' 
					) );
				}
	...
	function reply() {
		$bid = $this->spArgs ( 'bid' );
		$result = spClass ( 'db_replay' )->spLinker ()->spPager ( $this->spArgs ( 'page', 1 ), $this->spArgs ( 'limit', 10 ) )->findAll ( array (
				'bid' => $bid 
		), 'time desc', '' );
		$pager = '';
		$data = array ();
	...
	public function repblog() {
		...
		if ($rs == 1) {
			$this->api_success ( '内容已经成功转载', goUserHome ( array (
					'uid' => $this->uid 
			) ) );
		} elseif ($rs == - 2) {
	...
	public function vote() {
		...
		$blog = spClass ( 'db_blog' )->find ( array (
				'bid' => $this->spArgs ( 'bid' ) 
		) );
		...
		if (true == $db_voted->create ( $row ) && true == $db_blog->update ( array (
				'bid' => $this->spArgs ( 'bid' ) 
		), $blog )) {
	...
	function getHit() {
		$bid = $this->spArgs ( 'bid' );
		$result = spClass ( 'db_feeds' )->spLinker ()->spPager ( $this->spArgs ( 'page', 1 ), $this->spArgs ( 'limit', 10 ) )->findAll ( array (
				'bid' => $this->spArgs ( 'bid' ) 
		), 'id desc' );
		...
		if (! spAccess ( 'r', 'loginUserHot' )) { // 读取缓存
			...
			foreach ( $data as &$d ) {
				if (is_array ( $d ['user'] )) {
					...
					$d ['u_url'] = goUserHome ( array (
							'uid' => $d ['uid'],
							'domain' => $d ['user'] ['domain'] 
					) );
					$d ['u_img'] = avatar ( array (
							'uid' => $d ['uid'],
							'size' => 'middle' 
					) );
