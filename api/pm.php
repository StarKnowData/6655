...
class pm extends top {
	...
	function sendpm() {
		...
		$user = spClass ( 'db_member' )->find ( array (
				'username' => strreplaces ( $this->spArgs ( 'username' ) ) 
		) );
		if (! is_array ( $user )) {
	...
	function pminfo() {
		...
		$toname = spclass ( 'db_member' )->find ( array (
				'uid' => $touid 
		), '', 'username' );
		$num = '';
