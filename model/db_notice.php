...
class db_notice extends ybModel {
	...
	function sendReplay($imuid, $uid, $msga, $bid) {
		...
		$rs = spClass ( 'db_member' )->find ( array (
				'uid' => $uid 
		) );
		$for = spClass ( 'db_member' )->find ( array (
				'uid' => $imuid 
		) );
		if ($rs ['m_rep'] != 1)
			return true;
		...
		$msg = str_replace ( $msg [0], '<a href="' . $uri ['dirname'] . goUserHome ( array (
				'uid' => $msg [1] 
		) ) . '" target="_blank">' . $msg [2] . '</a>', $msga ); // 处理邮件中的at
		$tpl = '<h3>亲爱的' . $rs ['username'] . '：</h3>
			    <table width="100%" cellpadding="0" cellspacing="0" border="0" >
				    <tr>
					    <td width="50" valign="top"><img src="' . $uri . avatar ( array (
				'uid' => $imuid,
				'size' => 'small' 
		) ) . '" align="bottom"/></td>
						<td>
						    <p>' . $for ['username'] . '回复了您的轻博客：</p>
						    <p>"' . $msg . '"</p><a href="' . $uri . goUserBlog ( array (
				'bid' => $bid 
		) ) . '" target="_blank">你可以立即查看您的轻博</a></p>
						</td>
					</tr>
				</table>';
		...
		$tpl = '<h3>亲爱的' . $rs ['username'] . '：</h3>
			    <table width="100%" cellpadding="0" cellspacing="0" border="0" >
				    <tr>
						...
						<img src="' . $uri . avatar ( array (
				'uid' => $imuid,
				'size' => 'small' 
		) ) . '" /></td>
						<td>
						    <p>' . $for ['username'] . ',对您说：</p>
						    <p>"' . $info . '" <a href="' . $uri . spUrl ( 'user', 'pm', array (
				'look' => $imuid 
		) ) . '" target="_blank">你可以立即给Ta回复</a></p>
	...
	function sendFollow($uid, $imuid) {
		...
		$tpl = '<h3>亲爱的' . $rs ['username'] . '：</h3>
				...
				'size' => 'middle' 
						...
						<td>
						    <p>' . $for ['username'] . ',关注了你。<a href="' . $uri . goUserHome ( array (
				'domain' => 'home',
				'uid' => $imuid 
		) ) . '" target="_blank">你可以去看看Ta哟</a></p>
							<p>' . date ( 'Y-m-d H:i', time () ) . '</p>
						</td>
					</tr>
				</table>';
	...
	function findByall($uid) {
		return $this->spLinker ()->findAll ( array (
				'foruid' => $uid,
				'isread' => 0 
		), 'time desc' );
	}
