...
class db_blog extends ybModel {
	...
	function delblog($bid, $uid) {
		...
		spClass ( 'db_member' )->decrField ( array (
				'uid' => $uid 
		), 'num' ); // 计数减一
			            // 下面的由触发器完成
	...
	function blogrep($bid) {
		$rs = $this->spLinker ()->find ( array (
				'bid' => $bid 
		) );
		if (! $rs) {
			return - 1;
		}
		if ($rs ['uid'] == $_SESSION ['uid']) {
			return - 2;
		} // 自己的
		...
		if ($split ['repto']) {
			$repto = '[repto]' . serialize ( $split ['repto'] ) . '[/repto]';
		} else {
			$repto = '[repto]' . serialize ( $repto ) . '[/repto]';
		}
		if ($split ['attr']) {
			$attr = '[attribute]' . serialize ( $split ['attr'] ) . '[/attribute]';
		} else {
			$attr = '';
		}
		...
		if ($rs) {
			spClass ( 'db_member' )->incrField ( array (
					'uid' => $_SESSION ['uid'] 
			), 'num' ); // 我发布的统计+1
			spClass ( 'db_feeds' )->addRep ( array (
					'bid' => $bid 
			), $_SESSION ['uid'] );
			$this->create ( $rs );
			return 1;
