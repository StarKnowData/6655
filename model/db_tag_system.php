...
class db_tag_system extends ybModel {
	...
	function findTagByAttr($args, $uid) {
		...
		if ($args ['tag'] && $args ['tag'] != '') { // 根据参数为名称或id返回
			$where = array (
					'name' => strreplaces ( tagEncodeParse ( $args ['tag'] ) ) 
			);
		} else {
			$where = array (
					'id' => ( int ) $args ['tid'] 
		...
		if ($ret) {
			...
			$result = spClass ( 'db_tags' )->spPager ( $thispage, 15 )->findAll ( array (
					'title' => $ret ['name'] 
			) );
			...
			if (! empty ( $result )) {
				foreach ( $result as $d ) {
					if ($ret = $blogobj->spLinker ()->find ( array (
							'bid' => $d ['bid'] 
					) )) {
						$blog [] = $ret;
					}
				...
				$data ['isadd'] = (spClass ( 'db_mytag' )->find ( array (
						'tagid' => $data ['currtid'],
						'uid' => $uid 
				) )) ? 1 : 0; // 是否添加
			}
