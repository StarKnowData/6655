...
EOF;
	...
	function get_sp_sort($sort_id, $current_id) {
		...
		foreach ( $s_channel as $key => $ss_channel ) {
			if ($ss_channel ['id'] == $current_id)
				echo '<li  class="current"><a href="index.php?c=show&a=index&sid=' . $ss_channel ['fid'] . '&cid=' . $ss_channel ['id'] . '">' . $ss_channel ['typename'] . '</a></li>';
			else {
				$content = <<<EOF
...
EOF;
	...
	function get_jj_sort($sort_id, $fid) {
		...
		foreach ( $s_channel as $key => $ss ) {
			if ($ss ['id'] == $sort_id)
				echo '<li  class="current"><a href="index.php?c=show&a=index&sid=' . $ss ['id'] . '">' . $ss ['typename'] . '</a></li>';
			else
				echo '<li><a href="index.php?c=show&a=jujiao&sid=' . $ss ['fid'] . '&nid=' . $ss ['id'] . '">' . $ss ['typename'] . '</a></li>';
		}
	}
	/**
	 * 获取分类选项(下拉菜单)
	 ...
	 */
	// 2 1 1
	function get_sort($default = 0, $fid = 0, $n = 0) {
		...
		foreach ( $a_sort as $b_sort ) {
			if ($default == $b_sort ['id'])
				$content .= '<option value="' . $b_sort ['id'] . '" selected="selected">' . $k . $b_sort ['typename'] . '</option>';
			else
				$content .= '<option value="' . $b_sort ['id'] . '">' . $k . $b_sort ['typename'] . '</option>';
			$s_sort = $this->findAll ( "`fid`='$b_sort[id]'", '`order`' );
			foreach ( $s_sort as $m ) {
				if ($default == $m ['id'])
					$content .= '<option value="' . $m ['id'] . '" selected="selected">--' . $m ['typename'] . '</option>';
				else
					$content .= '<option value="' . $m ['id'] . '" >--' . $m ['typename'] . '</option>';
				$ss_sort = $this->findAll ( "`fid`='$m[id]'", '`order`' );
				foreach ( $ss_sort as $ss ) {
					if ($default == $ss ['id'])
						$content .= '<option value="' . $ss ['id'] . '" selected="selected">----' . $ss ['typename'] . '</option>';
					else
						$content .= '<option value="' . $ss ['id'] . '" >----' . $ss ['typename'] . '</option>';
				}
			}
		}
		return $content;
