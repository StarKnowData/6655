...
function spRun() {
	...
	if (FALSE != $GLOBALS ['G_SP'] ['view'] ['auto_display']) {
		$__tplname = $__controller . $GLOBALS ['G_SP'] ['view'] ['auto_display_sep'] . $__action . $GLOBALS ['G_SP'] ['view'] ['auto_display_suffix']; // 拼装模板路径
 ...
 *        	output 是否将内容输出
 * @param
 *        	show_trace 是否将使用spError对变量进行追踪输出
 */
function dump($vars, $output = TRUE, $show_trace = FALSE) {
	// 部署模式下同时不允许查看调试信息的情况，直接退出。
	if (TRUE != SP_DEBUG && TRUE != $GLOBALS ['G_SP'] ['allow_trace_onrelease'])
		return;
 ...
 *
 * @param
 *        	filename 需要载入的文件名或者文件路径
 * @param
 *        	auto_search 载入文件找不到时是否搜索系统路径或文件，搜索路径的顺序为：应用程序包含目录 -> 应用程序Model目录 -> sp框架包含文件目录
 * @param
 *        	auto_error 自动提示扩展类载入出错信息
 */
function import($sfilename, $auto_search = TRUE, $auto_error = FALSE) {
	if (isset ( $GLOBALS ['G_SP'] ["import_file"] [md5 ( $sfilename )] ))
	...
	if (TRUE == @is_readable ( $sfilename )) {
		...
		if (TRUE == $auto_search) { // 需要搜索文件
			...
			foreach ( array_merge ( $GLOBALS ['G_SP'] ['sp_include_path'], array (
					$GLOBALS ['G_SP'] ['model_path'] 
			), $GLOBALS ['G_SP'] ['include_path'] ) as $sp_include_path ) {
				// 检查当前搜索路径中，该文件是否已经载入
				if (isset ( $GLOBALS ['G_SP'] ["import_file"] [md5 ( $sp_include_path . '/' . $sfilename )] ))
					return TRUE;
 ...
 *
 * @param
 *        	method 数据存取模式，取值"w"为存入数据，取值"r"读取数据，取值"c"为删除数据
 * @param
 *        	name 标识数据的名称
 * @param
 *        	value 存入的值，在读取数据和删除数据的模式下均为NULL
 * @param
 *        	life_time 变量的生存时间，默认为永久保存
 */
function spAccess($method, $name, $value = NULL, $life_time = -1) {
	// 使用function_access挂靠点
	if ($launch = spLaunch ( "function_access", array (
			'method' => $method,
			'name' => $name,
			'value' => $value,
			'life_time' => $life_time 
 ...
 *        	class_name 类名称
 * @param
 *        	args 类初始化时使用的参数，数组形式
 * @param
 *        	sdir 载入类定义文件的路径，可以是目录+文件名的方式，也可以单独是目录。sdir的值将传入import()进行载入
 * @param
 *        	force_inst 是否强制重新实例化对象
 */
...
function spError($msg, $output = TRUE, $stop = TRUE) {
	if ($GLOBALS ['G_SP'] ['sp_error_throw_exception'])
 ...
 *
 * @param
 *        	configname 挂靠程序设置点名称
 * @param
 *        	launchargs 挂靠参数
 * @param
 *        	return 是否存在返回数据，如需要返回，则该挂靠点仅能有一个挂靠操作
 */
 ...
 *
 * @param
 *        	controller 控制器名称，默认为配置'default_controller'
 * @param
 *        	action 动作名称，默认为配置'default_action'
 * @param
 *        	args 传递的参数，数组形式
 * @param
 *        	anchor 跳转锚点
 * @param
 *        	no_sphtml 是否应用spHtml设置，在FALSE时效果与不启用spHtml相同。
 */
function spUrl($controller = null, $action = null, $args = null, $anchor = null, $no_sphtml = FALSE) {
	...
	// 使用挂靠点
	if ($launch = spLaunch ( "function_url", array (
