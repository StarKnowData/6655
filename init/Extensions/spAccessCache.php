 ...
 */
if (SP_VERSION < 2.5)
	spError ( 'spAccessCache扩展要求SpeedPHP框架版本2.5以上。' );
class spAccessCache {
	/**
...
class access_driver_saememcache {
	public $mmc = null;
	public function __construct() {
		if (! $this->mmc = memcache_init ())
			spError ( "SAE的memcache初始化失败！" );
	}
	public function get($name) {
		return memcache_get ( $this->mmc, $name );
	}
...
class access_driver_eaccelerator {
	public function __construct() {
		if (! function_exists ( 'eaccelerator_put' ))
			spError ( 'PHP环境未安装eAccelerator函数库！' );
	}
	public function get($name) {
...
class access_driver_db extends spModel {
	...
	public function set($name, $value, $life_time) {
		...
		if (FALSE !== $this->find ( array (
				'cachename' => $name 
		), NULL, 'cachename' )) {
			return $this->updateField ( array (
					'cachename' => $name 
			), 'cachevalue', $value );
		} else {
