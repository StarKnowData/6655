...
function goUserHome($params) {
	...
	if ($domain != '' && $domain != 'home') {
		return spUrl ( 'userblog', 'index', array (
				'domain' => $domain 
		) );
	} else {
		return spUrl ( 'userblog', 'index', array (
...
function goUserBlog($params) {
	...
	$uid = $params ['uid']; // 判断是否存在uid
	if ($bid) {
		return spUrl ( 'userblog', 'show', array (
				'bid' => $bid 
		) );
	}
}
...
function replay_preg($params) {
	...
	$msag = str_replace ( $msag [0], '<a href="' . goUserHome ( array (
			'uid' => $msag [1] 
	) ) . '" target="_blank">' . $msag [2] . '</a>', $msg );
	echo $msag;
}
...
function notice_preg($params) {
	...
	$url = explode ( '|', $msg );
	if ($url [0] == 'blog') {
		echo goUserBlog ( array (
				'bid' => $url [1] 
		) );
	}
	
	if ($url [0] == 'user') {
		echo goUserHome ( array (
				'uid' => $url [1] 
		) );
	}
}
...
function split_attribute($body) {
	...
	if ($matches [1]) {
		$data ['attr'] = mb_unserialize ( $matches [1] );
	} else {
		$data ['attr'] = '';
	}
	...
	if ($matches [1]) {
		$data ['repto'] = mb_unserialize ( $matches [1] );
	} else {
		$data ['repto'] = '';
	}
