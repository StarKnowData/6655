...
class yb_music extends basePostModel {
	...
	function uploadmedia() {
		...
		parent::uploadmedia ( $this->mconfig ['cfg'] ['uploadsize'], $this->mconfig ['cfg'] ['uploadtype'] );
	}
	function links() {
	...
	private function __loadMusicString($strings) {
		...
		if (is_array ( $music )) {
			...
			// 检查上传媒体的使用情况
			$result = spClass ( 'db_attach' )->findAll ( array (
					'bid' => $_SESSION ['tempid'],
					'uid' => $this->uid 
			), '', 'id,bid,mime' ); // 锁定用户文件,防止提交非自己的id以至于被删除
			if (is_array ( $result )) {
				// 整理本地文件
				foreach ( $result as $d ) {
					// 判断只有媒体文件才被处理
					if ($d ['mime'] == 'mp3' || $d ['mime'] == 'wma' || $d ['mime'] == 'mid') {
						$locadata [] = $d ['id'];
					}
				}
				// 计算差集,删除编辑器未使用媒体
				...
				if (is_array ( $compute )) {
					foreach ( $compute as $d ) {
						spClass ( 'db_attach' )->delBy ( $d, $this->uid );
					}
				}
			}
		}
	...
	private function _localMusicParse($id, $desc) {
		...
		if ($result ['uid'] == $this->uid) {
			if ($desc [$d] != '描述') {
				spClass ( "db_attach" )->update ( array (
						'id' => $id 
				), array (
						'blogdesc' => $desc 
				) );
