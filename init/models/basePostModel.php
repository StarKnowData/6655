...
abstract class basePostModel extends top {
	...
	function saved($bodypre) {
		...
		if ($bid != 0) { // 如果是编辑
			...
			if ($one = spClass ( 'db_blog' )->findBy ( 'bid', $bid )) {
				...
				if ($split_data ['repto']) {
					$repto = '[repto]' . serialize ( $split_data ['repto'] ) . '[/repto]';
				} else {
					$repto = '';
				}
				$mode = false;
	...
	public function myattach($bid = 0) {
		if ($bid == 0) {
			$rs = spClass ( 'db_attach' )->findAll ( array (
					'uid' => $this->uid,
					'bid' => 0 
			) );
			if ($rs) {
			...
			$this->attach = spClass ( 'db_attach' )->findAll ( array (
					'uid' => $this->uid,
					'bid' => $bid 
			), 'time desc' );
		}
	...
	private function myTagUsually($num = 10) {
		$this->myTagUsually = spClass ( 'db_tags' )->spCache ( 3600 )->findAll ( array (
				'uid' => $this->uid 
		), 'num desc', '', $num );
	}
